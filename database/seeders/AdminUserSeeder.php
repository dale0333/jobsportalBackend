...
class AdminUserSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // ==================== SECRETARIAT ====================
        if (DB::table('users')->where('email', 'secretariat@cdc.gov.ph')->doesntExist()) {
            DB::table('users')->insert([
                'user_type' => 'secretariat',
                'name' => 'Secretariat Office',
                'email' => 'secretariat@cdc.gov.ph',
                'email_verified_at' => now(),
                'password' => Hash::make('password123'),
                'address' => 'Clark Freeport Zone, Pampanga, Philippines',
            ...
            $this->command->info('✅ Secretariat user created: secretariat@cdc.gov.ph / password123');
        } else {
            $this->command->info('ℹ️ Secretariat user already exists.');
        }

        // ==================== EMPLOYER ====================
        if (DB::table('users')->where('email', 'employer@cdc.gov.ph')->doesntExist()) {
            $userId = DB::table('users')->insertGetId([
                'user_type' => 'employer',
                'name' => 'TechWorks Corporation',
                'email' => 'employer@cdc.gov.ph',
                'email_verified_at' => now(),
                'password' => Hash::make('password123'),
            ...
            $this->command->info('✅ Employer created: employer@cdc.gov.ph / password123');
        } else {
            $this->command->info('ℹ️ Employer user already exists.');
        }

        // ==================== JOB SEEKER ====================
        if (DB::table('users')->where('email', 'jobseeker@cdc.gov.ph')->doesntExist()) {
            $userId = DB::table('users')->insertGetId([
                'user_type' => 'job_seeker',
                'name' => 'Juan Dela Cruz',
                'email' => 'jobseeker@cdc.gov.ph',
                'email_verified_at' => now(),
                'password' => Hash::make('password123'),
                'telephone' => $faker->phoneNumber,
                'address' => $faker->address,
            ...
            $this->command->info('✅ Job Seeker created: jobseeker@cdc.gov.ph / password123');
        } else {
            $this->command->info('ℹ️ Job Seeker user already exists.');
        }

        // ==================== PESO SCHOOL ====================
        if (DB::table('users')->where('email', 'pesoschool@cdc.gov.ph')->doesntExist()) {
            DB::table('users')->insert([
                'user_type' => 'peso_school',
                'name' => 'PESO School Clark',
                'email' => 'pesoschool@cdc.gov.ph',
                'email_verified_at' => now(),
                'password' => Hash::make('password123'),
                'telephone' => $faker->phoneNumber,
            ...
            $this->command->info('✅ PESO School user created: pesoschool@cdc.gov.ph / password123');
        } else {
            $this->command->info('ℹ️ PESO School user already exists.');
        }

        // ==================== MANPOWER AGENCY ====================
        if (DB::table('users')->where('email', 'manpower@cdc.gov.ph')->doesntExist()) {
            DB::table('users')->insert([
                'user_type' => 'manpower_agency',
                'name' => 'Clark Manpower Agency',
                'email' => 'manpower@cdc.gov.ph',
                'email_verified_at' => now(),
                'password' => Hash::make('password123'),
                'telephone' => $faker->phoneNumber,
            ...
            $this->command->info('✅ Manpower Agency user created: manpower@cdc.gov.ph / password123');
        } else {
