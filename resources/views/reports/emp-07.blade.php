            ...
            <tbody>
                ...
                @forelse ($job->jobApplications as $app)
                    <tr>
                        ...
                        <td style="text-align:center;">{{ $app->status == 'hired' ? 'X' : '' }}</td>
                        <td style="text-align:center;">{{ $app->status == 'rejected' ? 'X' : '' }}</td>
                        <td style="text-align:center;">{{ $app->status == 'interview' ? 'X' : '' }}</td>
                        <td style="text-align:center;">{{ $app->status == 'withdrawn' ? 'X' : '' }}</td>
                    </tr>
                @empty
                    <tr>
...
{{-- <body>
    ...
    <table>
        <tr>
            <td colspan="8" class="position-title">
                FOR THE POSITION OF: {{ $job->title ?? '_________________________' }}</td>
            </td>
        </tr>
        <thead>
            <tr>
            ...
            @forelse ($records as $job)
                @foreach ($job->jobApplications as $app)
                    <tr>
                        ...
                        <td>{{ $app->jobSeeker->user->name ?? 'N/A' }}</td>
                        <td>{{ $app->jobSeeker->user->telephone ?? 'N/A' }}</td>
                        <td>{{ ($app->jobSeeker->education_level ?? '') . ' - ' . ($app->jobSeeker->field_of_study ?? '') }}
                        </td>
                        <td style="text-align:center;">{{ $app->status == 'hired' ? 'X' : '' }}</td>
                        <td style="text-align:center;">{{ $app->status == 'rejected' ? 'X' : '' }}</td>
                        <td style="text-align:center;">{{ $app->status == 'interview' ? 'X' : '' }}</td>
    ...
    <div class="footer-section text-center">
        <p style="font-size: 9px; font-weight: bold; margin: 0;">
            *Subject to the Privacy Sharing Agreement of CFZ Locators and Applicant, all personal data of the applicant
            shall be used only by the addressee company to process the application and possible employment of said
            applicant and not for any other purposes not agreed upon.
        </p>
    </div>
    ...
    <div class="company-info footer-section">
        <table width="100%">
            <tr>
                <td>
                    <strong style="display:inline-block; width:35%;">COMPANY NAME:</strong>
                    <span style="display:inline-block;">{{ $job->employer->user->name ?? 'N/A' }}</span>
                </td>
                <td>
                    <strong style="display:inline-block; width:40%;">DATE OF REQUEST:</strong>
            ...
            <tr>
                <td>
                    <strong style="display:inline-block; width:35%;">CONTACT NOS.:</strong>
                    <span style="display:inline-block;">{{ $job->employer->user->telephone ?? 'N/A' }}</span>
                </td>
                <td>
                    <strong style="display:inline-block; width:40%;">TO BE RETURNED ON:</strong>
                    <span style="display:inline-block;">{{ $job->deadline->format('m/d/Y') }}</span>
                </td>
            </tr>
            <tr>
                <td>
                    <strong style="display:inline-block; width:35%;">EMAIL ADD.:</strong>
                    <span style="display:inline-block;">{{ $job->employer->user->email ?? 'N/A' }}</span>
                </td>
                <td>
                    <strong style="display:inline-block; width:40%;">DATE RETURNED:</strong>
    ...
    <div class="footer-section text-center">
        <p style="font-size: 9px; font-weight: bold; margin: 0;">
            I hereby undertake to endorse the following job applications to CDC and submit status report on the remarks
            below for a maximum period of fifteen (15) working days only.
        </p>
    </div>
    ...
    <table width="100%" style="margin-top: 20px; ">
        <tr>
            <td width="60%" style="vertical-align: top; padding: 5px; border-collapse: collapse; border: none;">
                <div class="signature-line">
                    Prepared by: ____________________________<br>
                    Company's Representative<br>
                    Printed Name and Signature / Position
            ...
            <td width="40%" style="vertical-align: top; padding: 5px; border-collapse: collapse; border: none;">
                <div class="privacy-notice">
                    *Failure to return Form FM-CDC-CSRPD-07 shall temporarily refrain you from accessing services
                    provided
                    by the skills and placement section until compliance has been satisfactorily completed.
                </div>
            </td>
        </tr>
    </table>
